...
void timer1_init() {
    ...
    TCCR1A = 0x00;
    TCCR1B = ( 1 << CS12 ) | ( 0 << CS11 ) | ( 0 << CS10 ); // Одна секунда
    TCNT1 = 0x0000;
    OCR1A = 0x7A11;
    OCR1B = 0x0000;
    ICR1 = 0x0000;
...
uint8_t PID(float Set, float Value, float Kp, float Ki, float Kd)
{
	...
	if (Ki > 0) {
		Integ += (E_pid / Ki);
		Integ = constrainF( Integ, MIN_POWER, MAX_POWER ); // ограничиваем интегральную составляющую 0..100%
	}
	else Integ = 0;
...
int main() {
	...
	while(1) {
		...
		if(Flag.TempMes == 1) {
			...
			Temperature[Milk] += mapf(Temperature[Milk], 20.8, 63.82, 0.6, 3.4);	// Коррекция температуры.
#endif
